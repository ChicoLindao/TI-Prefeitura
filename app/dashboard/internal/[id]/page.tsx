...
export default function InternalMaintenanceDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("ATENÇÃO: Deseja apagar este chamado e todo o seu histórico permanentemente?")) return;
    setIsDeleting(true);
    const res = await fetch(`/api/internal/${id}`, { method: "DELETE" });
    if (res.ok) router.push("/dashboard/internal");
  ...
  const filteredLogs = data.logs.filter((log: any) => !log.action.includes('Status alterado para'));

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 transition-colors px-4 py-6 sm:p-8 print:p-0 print:bg-white print:text-black">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200 mb-8 print:hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" value={newLog} onChange={(e) => setNewLog(e.target.value)} placeholder="Ex: Formatação concluída..." className="flex-1 bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-700 placeholder-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent focus:outline-none transition-all" />
              ...
              <div className="space-y-5 print:space-y-1">
                {filteredLogs.map((log: any) => (
                  <div key={log.id} className="relative pl-8 print:pl-0 print:border-l-2 print:border-gray-400 print:border-b print:pb-2">
                    ...
                    <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl print:bg-transparent print:border-none print:p-0">
                      <p className="text-sm text-slate-700 print:text-xs print:text-black">{log.action}</p>
